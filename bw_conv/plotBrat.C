...
void plotBrat(std::string canname  = "can.eps", std::string blurb = "" ,double re = 14e-3, double im = 14e-3, double minmass = 3840e-3, double maxmass =3891e-3 ){


 RooRealVar m("m","m",minmass, maxmass);
  
 RooRealVar* gRe =new RooRealVar("gRe", "gRe", re ,  0.,1 );
 RooRealVar* gIm =new RooRealVar("gIm", "gIm", im, 0, 1);

 RooRealVar* thres = new RooRealVar("thres","thres", Braatan::m_D0  + Braatan::m_D0star, 0, 1);
 RooBraatan* brat = new RooBraatan("brat","brat" , m, *gRe, *gIm , *thres); 
...
void makeplots(){
  ...
  plotBrat("brat_bb_set1.eps", "Babar I", 38.8e-3,0 );
  plotBrat("brat_bb_set2.eps", "Babar II", 13.6e-3,15.5e-3 );  
  plotBrat("brat_be_set1.eps", "Belle I", 47.5e-3,0 );
  plotBrat("brat_be_set2.eps", "Belle II", 38.4e-3,12e-3 );  
}
