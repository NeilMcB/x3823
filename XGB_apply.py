    ...
    for run in list(D_CONFIGS.keys()):
        ...
        if args.opt_cut is None:
            ...
            for prob_threshold in cuts:
                ...
                signal_efficiency_mcj = float(df_train[(df_train['prob_'+run] > prob_threshold) & (df_train['cat']   == 'mc_x')].shape[0]) / float(df_train[df_train['cat']   == 'mc_x'].shape[0])
                signal_efficiency_mcx = float(df_train[(df_train['prob_'+run] > prob_threshold) & (df_train['cat']   == 'mc_p')].shape[0]) / float(df_train[df_train['cat']   == 'mc_p'].shape[0])
