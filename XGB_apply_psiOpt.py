...
def fit_doubleCB(a_mc_x, a_data, out_path, s_info='', save_fig=True):
    ...
    b_mass_mc     = ROOT.RooRealVar("b_mass_mc"   , "B mass MC [MeV]", 5220. , 5400.)
...
def main(args):
    ...
    for run in list(D_CONFIGS.keys()):
        ...
        s_load_branches.add('prob_'+run)
    s_load_branches.add('scaledmass')
    s_load_branches.add('mjpipi')
    s_load_branches.add('bplus_PT')
    ...
    d_run_info['sig_est_alldata'] = d_sig_est_alldata
    ...
    for run in list(D_CONFIGS.keys()):
        ...
        out_path_run = out_path+run+'/'
        if not os.path.exists(out_path_run):
            os.makedirs(out_path_run)
        ...
        if args.opt_cut is None:
            ...
            sig_effs_mcx   = []  # record signal efficiencies - on MC psi(2S) only
            sig_effs_all   = []  # record signal efficiencies
            sig_effs_pdata = []# # record signal efficiencies on psi(2S) range data
            ...
            df_data_p = df_data[(df_data['mjpipi'] < 3696) & (df_data['mjpipi'] > 3676)]
            s0_X = 184
            s0_p = float(fit_doubleCB(df_mc_p['scaledmass'].as_matrix(), df_data_p['scaledmass'].as_matrix(), out_path, s_info='psi(2S)_s0_est')['data_sig_yield'])
            for prob_threshold in cuts:
                ...
                signal_efficiency_p_data = float(d_datap_fit_cut['data_sig_yield']) / s0_p
                ...
                #sig_effs_pdata.append(signal_efficiency_p_data)
                bgr_rejs      .append(background_rejection)
                # Optimize cut
                eff = signal_efficiency_p_data
                ...
                cut_scores.append((s0_X * eff) / sqrt((s0_X * eff) + B))
            # Base on cut optimisation metric
            ...
            #d_run_info[run]['psi_signal_efficiency'] = sig_effs_pdata[cut_index]
            d_run_info[run]['all_signal_efficiency'] = sig_effs_all  [cut_index]
            d_run_info[run]['mcj_signal_efficiency'] = sig_effs_mcj  [cut_index]
            d_run_info[run]['mcx_signal_efficiency'] = sig_effs_mcx  [cut_index]
            d_roc_plot[run]['sig_effs'] = sig_effs_mcj
        ...
        fig.savefig(out_path_run+'cut_score.pdf')
        plt.close()
        ...
        leg.Draw('same')
        # Save
        c.SaveAs(out_path_run+'Mass_histogram_B_XGBcut.pdf')
        ...
        leg.Draw('same')
        # Save
        c.SaveAs(out_path_run+'BDT_answer.pdf')

        ### Plot variable distributions
        print('   *** Plotting training variable distributions ***')
        out_path_var = out_path_run+'dist_vars/'
        ...
        out_path_mcp_data = out_path_run+'mcp_v_data/'
        ...
        ### Plot comparison to MC data
        out_path_mcx_data = out_path_run+'mcx_v_data/'
        ...
        out_path_mc_mc = out_path_run+'mcp_v_mcx/'
        ...
        # Fit
        d_cut_fit = fit_doubleCB(a_cut_mc, a_cut_data, out_path_run, s_info='cut_data_plot')
        # Store params
        ...
        d_sig_est = fit_doubleCB(a_mc_x, a_data_x, out_path_run, s_info='x_signal_yield_est')
